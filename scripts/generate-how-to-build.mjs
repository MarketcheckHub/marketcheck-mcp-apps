...
const APPS = [
  ...
  },
  {
    ...
    segment: "Appraiser",
    toolName: null,
    description: "Macro-level automotive market intelligence dashboard powered by sold-vehicle summary data. Tracks market health across five lenses: KPI cards (total sold volume, average sale price, average days on market, and price vs MSRP %), fastest and slowest moving models with month-over-month comparison, biggest price movers by make/model, segment mix donut chart (body type share of total sold volume), brand residual value bar chart (average sale price as % of MSRP per brand), and a sortable state ranking table. All sections update in real time when period, geography, inventory type, body type, or fuel filters change. Segment Mix and Brand Residual Value use a two-tier data strategy: primary data from dedicated API calls, with automatic fallback aggregation from the make+model rows so real data always renders.",
    useCases: [
      { persona: "Market Analysts", desc: "Track national or state-level volume, pricing, and DOM trends over custom periods (30d–1Y) to identify market inflection points." },
      { persona: "OEM / Brand Teams", desc: "Monitor brand residual value (sale price as % of MSRP) across competitors to benchmark pricing strength and incentive impact." },
      { persona: "Auction Houses", desc: "Identify the fastest and slowest moving segments by body type and fuel type to plan lane assignments and reserve pricing." },
      { persona: "Auto Lenders", desc: "Assess state-level volume and average price trends to inform regional underwriting guidelines and collateral risk." },
      { persona: "Dealers", desc: "Spot price movers early — models with significant month-over-month price shifts signal inventory or demand changes to act on." },
    ],
    urlParams: [
      { name: "api_key", desc: "Your MarketCheck API key — triggers live data mode" },
      { name: "state", desc: "2-letter state code (e.g. CA, TX) — pre-selects the Geography filter and scopes all data to that state" },
      { name: "period", desc: "Time window: 30d | 60d | 90d | 6M | 1Y — pre-selects the Period filter" },
      { name: "inventory", desc: "Inventory type: New | Used | Both — pre-selects the Inventory filter" },
      { name: "body", desc: "Body type: All | SUV | Sedan | Truck — pre-selects the Body Type filter" },
      { name: "fuel", desc: "Fuel type: All | ICE | EV | Hybrid — pre-selects the Fuel filter" },
    ],
    inputParams: [
      { name: "state", type: "string", required: false, desc: "2-letter US state code for regional filtering" },
      { name: "period", type: "string", required: false, desc: "Time window: 30d, 60d, 90d, 6M, or 1Y" },
      { name: "inventoryType", type: "string", required: false, desc: "New, Used, or Both" },
      { name: "bodyType", type: "string", required: false, desc: "All, SUV, Sedan, or Truck" },
      { name: "fuelType", type: "string", required: false, desc: "All, ICE, EV, or Hybrid" },
    ],
    apiFlow: [
      { step: 1, label: "5 Parallel Sold-Summary Calls", apis: ["soldSummary"], parallel: true, note: "All five calls fire simultaneously: (1) make+model ranked by sold_count — current period, for fastest/slowest movers and price mover comparison; (2) make+model — prior period, for month-over-month delta; (3) body_type ranked by sold_count — segment mix donut chart; (4) make ranked by sold_count — brand residual value bar chart using average_sale_price / avg_msrp; (5) summary_by=state — state ranking table sorted by volume, price, or DOM" },
    ],
    renders: "KPI row (total sold, avg price with MoM delta, avg DOM, price vs MSRP %); Fastest Movers table (top 10 by volume with rank, make/model, sold count, avg price, avg DOM, MoM change); Slowest Movers table (top 10 by days on market); Price Movers table (biggest avg price shifts vs prior period, dollar and percent); Segment Mix interactive donut chart (up to 6 body types, click slice to filter); Brand Residual Value horizontal bar chart (up to 15 brands, color-coded by retention tier: >95% green, 90-95% blue, 85-90% yellow, <85% red); State Ranking sortable table (state abbreviation with full-name tooltip, avg price, volume, avg DOM — click any header to sort)",
  },
  {
